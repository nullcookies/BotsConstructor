        ...
        public void RegisterInNotificationSystem(int accountId, WebSocket webSocket)
        {
            ...
            lock (lockObj)
            {
                ...
                else
                {
                    ...
                    if (!addIsOk)
                    {
                        _logger.Log(LogLevelMyDich.ERROR, ErrorSource.WEBSITE, $"Сайт. Сервис подсчёта заказов. Не удалось добавить webSocket для аккаунта accountId={accountId},");
                    }
                }
                ...
                for (int i = 0; i < the_bot_ids_on_which_the_account_is_signed.Count; i++)
                {
                        ...
                        && accounts_that_are_subscribed_to_bot.Count > 0)
                    {
                        ...
                        {
                            _logger.Log(LogLevelMyDich.INFO, ErrorSource.WEBSITE, "К словарю (accountId, List<WebSocket> ) добавлен сокет для аккаунта, который уже существует. Значит открыто несколько вкладок.");
                        }
                    }
                    else
                    {
